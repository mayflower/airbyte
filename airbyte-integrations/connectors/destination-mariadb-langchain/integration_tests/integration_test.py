...
import psycopg2
from destination_mariadb_langchain.destination import DestinationMariaDB
...
class MariaDBIntegrationTest(BaseIntegrationTest):
    ...
    def test_check_valid_config(self):
        outcome = DestinationMariaDB().check(logging.getLogger("airbyte"), self.config)
        assert outcome.status == Status.SUCCEEDED

    def test_check_invalid_config(self):
        outcome = DestinationMariaDB().check(
    ...
    def test_write_and_replace(self):
        ...
        # lol what? That above is original Airbyte stuff
        assert self._get_record_count("mystream") == 6
