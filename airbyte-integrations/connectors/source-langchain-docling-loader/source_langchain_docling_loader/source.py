...
SUPPORTED_EXTENSIONS = [
    ...
    '.csv',                                 # CSV
    '.png', '.jpg', '.jpeg', '.tiff', '.tif', '.bmp' # Image formats
]
...
class DoclingStream(CheckpointMixin, Stream):
    ...
    def _is_supported_file(self, file_path: str) -> bool:
        ...
        # Support all text/* MIME types (covers Markdown, AsciiDoc, etc.)
    ...
    def stream_slices(self, stream_state: Mapping[str, Any] = None, **kwargs) -> Iterable[Optional[Mapping[str, Any]]]:
        ...
        """
        # Initialize state from stream_state for incremental sync
    ...
    def _process_files_incremental(self, files_to_process: List[Dict[str, str]]) -> Iterable[StreamData]:
        ...
        self.logger.info(
            f"Extracted {record_counter} records from {processed_counter} files "
            f"(out of {len(files_to_process)} total files)"
        )
